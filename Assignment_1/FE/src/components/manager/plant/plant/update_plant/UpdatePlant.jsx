...
const UpdatePlant = ({ setShowModal, itemUpdate, setRefreshData }) => {
  ...
  const setDataItem = (itemUpdate) => {
    ...
    setHeatZone(itemUpdate.heat_zone)
    setSize(itemUpdate.size)
    setPrice(itemUpdate.price)
    setPlantType(itemUpdate.plant_typeId)
    setIsSeed(itemUpdate.is_seed)
  }
  ...
  const handleOnClick = async () => {
    ...
    try {
      const response = await axios.put(
        `${baseUrl}/plant_type/${plant_type.id}/plant/${itemUpdate.id}`,
        plant,
        {
